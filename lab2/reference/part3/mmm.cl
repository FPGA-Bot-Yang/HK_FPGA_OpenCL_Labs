...
__kernel void mmm(__global const float* restrict in_real, __global const float* restrict in_img, __global float* restrict out_real, __global float* restrict out_img, __global int* restrict N) {
	...
	float stage0_i[8];
    	...
    	for (int base = 0; base < 4; base += 1) {
    	    int j = base/m;
	    float c0_r = stage1_r[base];
	    float c0_i = stage1_i[base];
	    float c1_r = stage1_r[base+4];
	    float c1_i = stage1_i[base+4];
	    stage2_r[base+j*m] = c0_r+c1_r;
	    stage2_i[base+j*m] = c0_i+c1_i;
	    float c2_r = c0_r-c1_r;
	    float c2_i = c0_i-c1_i;
	    stage2_r[base+(j+1)*m] =w_r[j*4/L]*c2_r - w_i[j*4/L]*c2_i;
	    stage2_i[base+(j+1)*m] =w_r[j*4/L]*c2_i + w_i[j*4/L]*c2_r;
    	}
    	...
    	for (int base = 0; base < 4; base += 1) {
    	    int j = base/m;
	    float c0_r = stage2_r[base];
	    float c0_i = stage2_i[base];
	    float c1_r = stage2_r[base+4];
	    float c1_i = stage2_i[base+4];
	    stage3_r[base+j*m] = c0_r+c1_r;
	    stage3_i[base+j*m] = c0_i+c1_i;
	    float c2_r = c0_r-c1_r;
	    float c2_i = c0_i-c1_i;
	    stage3_r[base+(j+1)*m] =w_r[j*4/L]*c2_r - w_i[j*4/L]*c2_i;
	    stage3_i[base+(j+1)*m] =w_r[j*4/L]*c2_i + w_i[j*4/L]*c2_r;
    	}
